...
readme.write("# SibSUTIS CTF 2016\n")
...
possible_categories = ["admin", "web", "pwn", "joy", "crypto", "forensic", "misc", "ppc", "recon", "reverse", "stego"]
...
def getSolveFromTask(data, folder):
	solve = {"RU" : "", "EN": ""}
	if 'name' not in data:
		append_errors(folder, 'main.json: Missing field "name"')
	else:
		solve = data['solve']
		if 'RU' not in solve:
			append_errors(folder, 'main.json: Missing subfield solve "RU"')
		else:
			if solve["RU"] == "":
				append_errors(folder, 'main.json: Empty field in solve "RU"')

		if 'EN' not in solve:
			append_errors(folder, 'main.json: Missing subfield solve "EN"')
		else:
			if solve["EN"] == "":
				append_errors(folder, 'main.json: Empty field in solve "EN"')
	return solve
...
def getFlagKeyFromTask(data, folder):
	...
	if 'flag_key' not in data:
		...
		pattern = re.compile('SCTF\{.*\}')
		m = pattern.match(flag_key)
		if flag_key == "":
			append_errors(folder, 'main.json: Field "flag_key" is empty')
		elif not m:
			append_errors(folder, 'main.json: Wrong value of field "flag_key" must be format "SCTF{`md5`} or SCTF{`sometext`}"')
	return flag_key
	...
	if os.path.isfile(path):
		try:
			...
			with open(path) as main_json:
				data = json.load(main_json, encoding="utf-8")
